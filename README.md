...
### 步驟 B：客戶端驗證與讀取 (唯讀解密)
您可以選擇使用 **C# 視覺化 GUI** 或 **命令列測試程式** 進行唯讀解密驗證（均**完全不需要系統管理員權限**）：

#### 方法一：使用 C# GUI 進行視覺化驗證 (推薦 DEMO 展示)
1. 將製妥的隨身碟插入電腦。
2. 以**一般使用者身分**雙擊執行 `dongle_maker_gui.exe` (不跳 UAC 提示)。
3. 切換至第二分頁 **「讀取與驗證 (Reader Demo)」**。
4. 選擇對應的隨身碟，並輸入您製鎖時設定的密碼。
5. 點擊 **「讀取並驗證軟體鎖 (Demo)」** 按鈕。
6. 程式會自動調用 `libDongleReader.dll` 安全讀取晶片 ID，並在記憶體中還原顯示您的授權 JSON 內容。

#### 方法二：使用命令列程式測試
1. 開啟一般終端機。
2. 切換至讀取編譯目錄並執行 `ReaderConsole.exe`：
   ```powershell
   cd d:\dongle_maker\cpp_src\build\reader
   .\ReaderConsole.exe
   ```
3. 輸入磁碟號並輸入您的製鎖密碼，即可看到解密明文。
